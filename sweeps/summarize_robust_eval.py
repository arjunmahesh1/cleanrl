...
def main():
    ...
    parser.add_argument("--robust-model-label", default="tv")
    parser.add_argument("--nominal-scenario-label", default="nominal")
    parser.add_argument("--perturbed-scenario-label", default="")
    parser.add_argument("--perturbed-scenario-prefix", default="")
    ...
    all_scenarios = sorted({key[1] for key in latest.keys()})
    if args.perturbed_scenario_label:
        perturbed_scenarios = [args.perturbed_scenario_label]
    elif args.perturbed_scenario_prefix:
        perturbed_scenarios = [s for s in all_scenarios if s.startswith(args.perturbed_scenario_prefix)]
    else:
        perturbed_scenarios = [s for s in all_scenarios if s != args.nominal_scenario_label]
    ...
    for perturbed_scenario in perturbed_scenarios:
        drops_by_model = defaultdict(list)
        for model in models:
            for seed in seeds:
                nominal = latest.get((model, args.nominal_scenario_label, seed))
                perturbed = latest.get((model, perturbed_scenario, seed))
        ...
        print(f"\nDrop summary for scenario={perturbed_scenario} (nominal - perturbed, lower is better):")
        for model, drows in sorted(drops_by_model.items()):
            dm = [x["drop_mean"] for x in drows]
            dmed = [x["drop_median"] for x in drows]
            rd = [x["rel_drop_mean"] for x in drows]
            ...
            summary_rows.append(
                {
                    "kind": "drop",
                    "model_label": model,
                    "scenario_label": f"{args.nominal_scenario_label}->{perturbed_scenario}",
                    "n": len(drows),
